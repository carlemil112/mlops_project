...
unlearned.load_state_dict(
    torch.load("MNIST task/exercise_2/task2_unlearned_model.pth", map_location=DEVICE)
)
...
for digit in range(10):
    ...
    print(
        f"  {digit:<6} {baseline_acc[digit]:>9.2f}% {unlearned_acc[digit]:>9.2f}% {change:>+9.2f}%{marker}"
    )
...
bars_base = ax.bar(
    [i - 0.2 for i in x],
    [baseline_acc[d] for d in digits],
    width=0.4,
    label="Baseline",
    color="steelblue",
)
bars_unl = ax.bar(
    [i + 0.2 for i in x],
    [unlearned_acc[d] for d in digits],
    width=0.4,
    label="Unlearned",
    color="tomato",
)
...
ax.set_ylim(0, 105)
ax.set_title(
    f"Exercise 2: Gradient Ascent Unlearning – Digit {FORGET_CLASS} as Forget Target"
)
ax.legend()
