...
class MemoryBuffer:
    ...
    def fill(self, dataset):
        ...
        print(f"  Memory buffer: {total} samples across classes {sorted(self.store.keys())}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    print(f"Epoch {epoch}/{EPOCHS}  –  old(0-4): {acc_old:.2f}%  |  new(5-9): {acc_new:.2f}%")

# Plot 
epochs = range(1, EPOCHS + 1)
plt.figure(figsize=(8, 5))
plt.axhline(TASK1_BASELINE, color="green", linestyle="--", label=f"Task-1 baseline ({TASK1_BASELINE}%)")
