...
def run_conversion():
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"convert_model.py failed with exit code {result.returncode}")
...
def log_to_mlflow(run_id, pytorch_acc, pytorch_latency,
                  tflite_acc, tflite_latency, tflite_size_kb):
...
def main():
    ...
    parser.add_argument("--run-id", required=True,
                        help="MLflow run ID from the training stage")
    parser.add_argument("--config-dir", default=os.path.join(os.getcwd(), "configs"),
    ...
    print(f"\nDone.", flush=True)
    print(f"  PyTorch  acc={pytorch_acc:.4f}  latency={pytorch_latency:.2f} ms", flush=True)
    print(f"  TFLite   acc={tflite_acc:.4f}  latency={tflite_latency:.2f} ms", flush=True)
